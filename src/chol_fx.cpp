...
//' Cross-distance between two matrices (RcppEigen version)
 ...
 Eigen::SparseMatrix<double> crossDist_sparse(
   ...
   // --- Build triplets directly from D ---
   // (Previously this used hand-rolled flat-index arrays built from
   // .replicate()/.reshaped() to locate entries in D's column-major
   // storage; the column-index array was built by tiling a length-m
   // vector n times (period m) instead of repeating each column index n
   // times in a row (period n), so it did not match D's actual
   // column-major layout and entries were written to the wrong (row,
   // col) position -- e.g. a nonzero off-diagonal distance would end up
   // stored at a diagonal position. Iterating (row, col) directly avoids
   // that index bookkeeping entirely.)
   std::vector<Eigen::Triplet<double>> triplets;
   triplets.reserve(static_cast<std::size_t>(n) * static_cast<std::size_t>(m));
   for (int j = 0; j < m; ++j) {
     for (int i = 0; i < n; ++i) {
       double dij = D(i, j);
       if (dij <= rmax) {
         triplets.emplace_back(i, j, dij);
       }
     }
   }
